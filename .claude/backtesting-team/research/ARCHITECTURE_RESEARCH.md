...
- Source class: official-doc
- Claim class: mechanics-sensitive
- Relevant claim (missed in the original build pass): the `Agent(agent_type, ...)` parenthetical allowlist syntax in a `tools:` field **only restricts spawnable subagent types when that agent runs as the main session** (`claude --agent <name>`). When the same file is invoked as an ordinary subagent (the documented, README-recommended way to use `bt-team-orchestrator`), Claude Code ignores the parenthetical list entirely — listing bare `Agent` in a subagent's `tools:` permits spawning any subagent type, subject only to the spawn-depth limit.
- Applies to: `bt-team-orchestrator`'s tool contract and its "cannot spawn arbitrary subagents" claim.
- Design consequence: the original build incorrectly treated `Agent(bt-methods-researcher, ...)` as a runtime-enforced restriction for a subagent-invoked orchestrator. It is not. `bt-team-orchestrator.md` has been corrected to (1) state this plainly rather than retain the false claim, and (2) carry the 12-role restriction as an explicit prompt-level operating invariant instead, since no runtime sandboxing mechanism currently enforces it for this invocation mode.
- Limitations/disagreement: none — this is a direct reading of the current official documentation the original build pass fetched but did not fully extract.
...
| Purging/embargo/CPCV as the hard gate for temporal-leakage review | secondary (corrected 2026-09-17 — only secondary summaries of López de Prado 2018 were actually inspected; see `BUILD_RESEARCH.md` shared evidence entry 1) | López de Prado 2018 *Advances in Financial Machine Learning*, ch. 7 & 12 (cited; not directly inspected) | High for the purge/embargo principle as restated consistently across independent secondary sources; not independently re-derived from the primary text | `bt-feature-label-methodologist`, `bt-experiment-designer`, `bt-experiment-integrity-reviewer`, temporal-leakage-audit skill | Repository-specific label horizon/embargo width remains unresolved until real data frequency is known |
| Deflated Sharpe Ratio / Probability of Backtest Overfitting cited to justify the mandatory trial-count-disclosure gate (not implemented — the bundled `multiple-testing-correction` skill only performs generic Bonferroni/BH correction; see its scope-boundary note) | secondary (corrected 2026-09-17 — the DSR/PBO/Harvey-Liu-Zhu papers were not directly inspected; see `BUILD_RESEARCH.md` shared evidence entries 2–3) | Bailey & López de Prado 2012 (Probabilistic Sharpe Ratio), 2014 (Deflated Sharpe Ratio); Bailey, Borwein, López de Prado & Zhu 2017 (Probability of Backtest Overfitting, cited but not separately inspected in this build); Harvey, Liu & Zhu 2016 (multiple-testing problem in empirical finance) | Medium — the general direction of each claim is corroborated across independently-inspected secondary sources; no formula-level or threshold-level detail was independently verified | `bt-statistical-reviewer`, `bt-experiment-integrity-reviewer`, multiple-testing-correction skill | Exact significance/FDR threshold left as a documented, non-universalized choice the user must set per study; DSR/PBO implementation itself is out of scope for this build's bundled script, not merely unresolved |
